...
printf("\n");
  for (int i = 0; i < 3 ; i++){
    for(int j = 0; j< 3; j++){
      printf("%x %d\t",(ap+i*3+j), *(ap+i*3+j)); //*(ap+i+j) provoca que se duplique informaciòn
                                  //i*3 multiplica al numero de elementos dentro el renglon (elementos de cada renglon) 
